...
- 일정한 기준에 따라 각 기물의 행동 별 획득 가능한 점수 연산 후 가장 높은 점수를 획득하는 행동 선별, 최고점수 행동을 수행 

## 점수 연산 기준

- 각 기물별로 점수 할당 : 각 기물별로 기존 체스의 기물 가치를 반영해 점수 할당
- 영향력 : 각 좌표에 가치를 부여, 중앙일수록, 상대 진영일수록 높은 가치, 기물이 개입 가능한 좌표들의 가치만큼 합산
...
- 매 턴마다 연산 최적화를 위해 플레이어에게 보이지 않는 별개의 체스보드를 가상으로 만들어 기물의 위치, 행동별 점수들을 저장, 게임 내 변화 발생 시 연산 로직에 근거해 행동별 점수들에 반영
