...
## 260903 - 카드 드로우 2차 베지어 포물선(Arc) 비행 & Tween 연출 일원화
*   **작업 내용**:
    *   **카드 드로우 연출의 2차 베지어(Quadratic Bezier) 아치 곡선 및 Tween 일원화 (`CardManager.gd`)**:
        *   **문제 해결**: 기존 `_process` 기반의 `lerp(12.0 * delta)`로 인해 이동 거리가 먼 첫 번째 카드(우측 끝 배치)가 초음속으로 튕겨 나가 덱이 아닌 중간에서 날아오는 것처럼 느껴지던 시각적 착시 현상 완벽 해결.
        *   **2차 베지어 포물선(Arc) 비행**: 카드가 덱에서 나올 때 상공(+0.35m)으로 살짝 솟아올라 우아한 호를 그리며 손패 슬롯으로 미끄러져 들어가는 입체적인 드로우 궤적 구현.
        *   **3대 속성 동기화 (위치 + 회전 + 크기)**: `is_drawing` 플래그로 비행 중 `_process` 간섭을 완벽 차단하고, 0.5초 동안 `EASE_OUT` / `TRANS_CUBIC` 감속 곡선을 타며 180도 뒤집기 및 1.0배 정크기 확대가 한 호흡에 스무스하게 착지하도록 일원화 완료.
    *   **덱 / 버린 카드 더미 뷰어 UI 최상위 윈도우 캔버스(layer=50) 이관 및 보드판 조작 3중 차단 (`BoardInput.gd`, `CardManager.gd`)**:
        *   **문제 해결**: 기존 뷰어 UI(`viewer_canvas`)가 3D 서브뷰포트 내부에 생성되어, 메인 윈도우에 직접 렌더링되던 우측 상단 "현재 위치" 및 "행동가능 기물" HUD 라벨이 뷰어의 암전 배경을 뚫고 최상단에 노출되던 결함 해결.
        *   **최상위 부착 및 레이어 통제**: 뷰어 캔버스를 `get_tree().root.add_child(viewer_canvas)`로 직행 생성하고 `layer = 50`을 부여하여, 85% 불투명 블랙 암전 배경이 메인 배틀 HUD 전체를 완전히 덮도록 개선.
        *   **체스 보드 입력 3중 원천 차단 (`BoardInput.gd`, `CardManager.gd`)**:
            1. `CardManager.execute_card_view()`에서 누락되었던 `current_state = State.VIEWING` 상태 전환 코드를 정상 복구하고, 뷰어 진입 즉시 기존 체스판 호버 아웃라인 및 타일 정보를 강제 소거(`_set_piece_outline(false)`).
            2. `BoardInput.gd`의 `_physics_process`(호버/아웃라인) 및 `_unhandled_input`(클릭/이동)에서 인스턴스 대신 전역 정적 클래스 상수(`CardManager.State.IDLE`)를 정확히 비교하여 뷰어 상태 시 체스 기물 하이라이트 감지를 100% 원천 차단.
            3. 뷰어 배경(`bg`) 및 스크롤 영역(`scroll`)에 `mouse_filter = Control.MOUSE_FILTER_STOP`을 부여하여 클릭 신호가 뒤쪽 3D 체스판으로 새어 나가는 현상 봉쇄.
            4. 뷰어 진입 시 `board_input.clear_selection()`을 자동 호출하여 기존에 선택 중이던 기물 아웃라인과 이동 닷을 즉시 정리.
        *   **안전한 메모리/상태 해제**: `active_viewer_canvas` 참조 관리 및 `_exit_tree()` 정리 로직을 추가하여 씬 전환이나 닫기 시 잔여 노드 없이 깨끗하게 제거 보장.
    *   **리워드 화면 및 카드 뷰어 버튼 UI 타이틀 디자인 시스템 일체화 (`RewardUI.gd`, `CardManager.gd`)**:
        *   **리워드 화면 4종 버튼 스타일링 (`RewardUI.gd`)**: `[덱에서 카드 1장 삭제]`, `[덱 유지하고 1스테이지 재도전]`, `[다음 스테이지로 ➔]`, 삭제 팝업 `[닫기]` 버튼에 타이틀 화면과 동일한 다크 네이비 88% 배경(`Color(0.10, 0.12, 0.18, 0.88)`), 선명한 2px 실버 테두리, 10px 라운딩, 마우스 호버 시 황금빛 폰트 발광(`Color(1.0, 0.92, 0.5)`) 및 순백색 테두리 인터랙션을 일괄 적용.
        *   **핵심 액션 포인트 악센트**: `[다음 스테이지로 ➔]` 버튼에 골드 테두리 및 틴트 배경을 부여하고, `[재도전]` 버튼에 시안 테두리를 부여하여 선택지의 시각적 위계 명확화.
        *   **카드 뷰어 닫기 버튼 레이아웃 정돈 (`CardManager.gd`)**: 가로로 과도하게 늘어나 있던 하단 닫기 버튼을 타이틀 버튼 비율과 일치하는 중앙 정렬 컴팩트 알약 형태(`240px × 52px`)로 재배치하고, 호버/프레스드/비활성화 상태를 완벽 동기화.
    *   **리워드 카드 1회 삭제 완료 시 물리적 잠금(Disabled) 및 피드백 강화 (`RewardUI.gd`)**:
        *   **문제 해결**: 카드 1장 삭제 후 보상 카드를 수령했을 때 람다 스코프 문제 등으로 인해 삭제 팝업이 다시 열릴 수 있던 잠재 결함 해결.
        *   **버튼 물리 비활성화 (`disabled = true`)**: 카드를 삭제하는 즉시 `remove_btn.disabled = true`를 적용하여 고도 엔진 차원에서 클릭 이벤트를 원천 차단하고 `style_disabled`(반투명 다크 그레이)를 발동.
        *   **UX 텍스트 피드백**: 버튼 텍스트를 `"카드 1장 삭제 완료"`로 변경하여 플레이어가 이번 스테이지 보상 기회를 모두 소진했음을 명확히 인지하도록 개선.

---

## 260902 - 3D 카드 아우라 쉐이더, 드래그 프레임 최적화, UI 스타일 통일 & 런 세이브 보존
*   **작업 내용**:
    *   **덱 / 버린 카드 더미 뷰어 UI 비율 보정 및 중앙 정렬 연출 (`CardManager.gd`)**:
        *   **2:3 표준 비율 동기화 (`180 × 270`)**: 2D 뷰어 카드의 최소 크기를 `180px × 270px` (정확한 2:3 비율)로 수정하여 비율 왜곡/찌그러짐 완벽 방지.
        *   **CenterContainer 화면 중앙 정렬**: 카드 수량(1장~10장 이상)과 상관없이 모든 카드가 화면 중앙에 깔끔하게 정렬되도록 레이아웃 구조 개편.
        *   **마우스 호버 애니메이션 연동**: 2D 뷰어 카드에 마우스 호버 시 `1.18배` 시원하고 선명하게 확대(`Tween`)되는 모션 연출 강화.
        *   **UI 텍스트 및 닫기 버튼 정돈**: 닫기 버튼 텍스트를 깔끔하게 `"닫기"`로 간결화.
        *   **최상위 루트 창 CanvasLayer(layer=100) 전환**: 카드 뷰어 UI(`viewer_canvas`)를 서브뷰포트 내부가 아닌 `get_tree().root` 최상위 윈도우의 `CanvasLayer(layer = 100)`으로 직행 생성하도록 개편하여, `현재 위치: None` 및 `행동가능 기물` 등 메인 창의 모든 HUD UI가 뷰어 암전 배경 아래로 100% 가려지도록 통제 완료.
    *   **턴 종료 시 행동권 토큰 소멸 및 보드 선택 해제 (`CardManager.gd`, `BoardInput.gd`)**:
        *   **턴 종료 토큰 리셋 시점 당김 (`bm.reset_all_action_tokens()`)**: `CardManager.end_turn()` 실행 직후 남아있는 모든 기물 행동권을 즉시 `0`으로 소멸시켜 상대 턴 동안 기물이 클릭/이동되는 문제 근본 해결.
        *   **보드 선택 상태 및 아웃라인 초기화 (`BoardInput.clear_selection()`)**: 턴 종료 시 선택 중이던 기물의 노란색 하이라이트 및 이동 유효 타일 표시를 깨끗이 해제.
    *   **런 라이프사이클 및 저장 데이터 정리 (`ProfileManager.gd`, `TitleManager.gd`, `StageManager.gd`, `BoardManager.gd`)**:
        *   **HP 항목 제거**: 사용되지 않는 `hp` 항목을 프로필 구조 및 런 데이터에서 완전히 제거.
        *   **런 시작 시점 개편 (`start_active_run`)**: `[새 게임]` 버튼 클릭 시에는 런 데이터만 준비(`is_in_run = false`)하고, 실제 스테이지 버튼을 클릭해 전투에 진입하는 시점(`StageManager._transition_to_battle`)에 런 시작을 공식 확정(`is_in_run = true`, `total_runs += 1`).
        *   **패배 시 런 종료 처리 (`BoardManager.trigger_defeat`)**: 플레이어 왕 파괴로 패배 시 `ProfileManager.clear_current_run()`을 호출하여 휘발성 런 데이터를 초기화하고 런을 공식 종료.
    *   **리워드 화면 덱 카드 삭제 호버 애니메이션 및 레이아웃 확장 (`RewardUI.gd`)**:
        *   **마우스 호버 확대 연출 (`1.18배`)**: `_create_removal_card_panel`의 카드 패널에 중앙 피벗(`Vector2(80, 110)`) 및 호버 `Tween` 애니메이션을 연동하여, 카드 삭제 팝업에서 마우스 호버 시 카드가 부드럽고 선명하게 1.18배 확대되도록 UX 개선.
        *   **팝업 위치 상향 및 위아래 세로 확장 (`1180 × 820px`, 스크롤 `670px`)**: 팝업 패널을 화면 위쪽으로 살짝 올려서 정렬하고 스크롤 가능 높이를 기존 `420px`에서 `670px`로 시원하게 확장하여 답답함 해소.
    *   **게임 전체 주요 버튼 UI 디자인 통일 및 스타일링 고급화 (`TitleManager.gd`, `StageManager.gd`, `PauseUI.gd`, `CardManager.gd`)**:
        *   **타이틀 화면 4개 버튼**: `NewGame`, `Continue`, `Option`, `Quit` 버튼에 88% 불투명 다크 네이비 배경(`Color(0.10, 0.12, 0.18, 0.88)`), 선명한 2px 흰색 테두리(`border_color = Color(0.92, 0.92, 0.96, 0.9)`), 둥근 모서리(`10px`) 및 그림자 효과 연동.
        *   **스테이지 선택 화면 버튼**: `Stage1`, `Stage2`, `Stage3` 및 `← 뒤로가기` 버튼에 불투명 배경 및 3px 흰색/골드 하이라이트 테두리를 적용하여 명확한 버튼 시각성 확보.
        *   **일시정지 화면 4개 버튼 (`PauseUI.gd`)**: `Resume`, `Option`, `Title`, `Quit` 버튼에 88% 불투명 다크 네이비 배경, 2px 흰색 테두리 및 1.05배 부드러운 호버 확대 모션 연동.
        *   **덱/버린 카드 뷰어 닫기 버튼 및 턴 종료 버튼 (`CardManager.gd`)**: 뷰어 모달 하단 `닫기` 버튼 및 `턴 종료 [Space]` 버튼(은은한 골드 틴트 강조)에 불투명 배경 및 선명한 테두리를 일괄 연동하여 전체 UI 버튼 룩앤필 완성.
    *   **카드 드래그 사용 영역(Play Area) 실시간 소프트 글로우 3D 아우라 구현 (`CardVisual3D.gd`, `CardManager.gd`)**:
        *   **SDF 라운디드 박스 3D 쉐이더(`sd_round_box`)**: 2D/3D SDF 둥근 사각형 연산 공식을 탑재하여 4개 모서리가 카드의 곡면을 따라 동글동글하고 매끄럽게 라운딩(`corner_radius = 0.07`)되는 황금빛 3D 마법 아우라 연출 완성.
        *   **실시간 영역 및 코스트 조건 판정 (`_can_play_card`)**: 마우스 드래그 중 Y 좌표가 화면 상단 60%(Play Area) 진입 **AND** 보유 코스트가 필요 코스트 이상일 때만 발광 아우라 발동. 코스트가 부족한 카드는 상단으로 드래그하더라도 아우라가 켜지지 않아 플레이어가 사전에 코스트 부족을 명확히 인지하도록 UX 개선.
        *   **`_process(delta)` 렌더링 프레임 동기화 드래그 최적화 (`CardManager.gd`)**: 기존 `InputEventMouseMotion` 기반의 미세 끊김(Stuttering)을 해결하기 위해 `_process(delta)`에서 매 프레임 모니터 주사율(60~144Hz)에 맞춰 실시간 35.0 lerp 추적하도록 개선하여 극상으로 부드럽고 찰진 드래그 손맛 구축.
    *   **신규 런 세이브 라이프사이클 보존 수정 (`TitleManager.gd`, `StageManager.gd`)**:
        *   기존 New Game 버튼 클릭 시 세이브 데이터가 즉시 초기화되던 문제를 해결하여, 실제 스테이지 1/2/3 진입 확정 시점에 런 생성이 시작되도록 이관.
        *   스테이지 선택 화면 진입 후 `← 뒤로가기`로 복귀하더라도 진행 중이던 기존 런 데이터 및 `이어서 하기(Continue)` 버튼 활성화 상태가 100% 안전하게 유지되도록 보장.
...
*   **작업 내용**:
    ...
    *   **전술 스킬 카드 [랜스 차징 / Lance Charge] 메커니즘 고도화 (`ChessRules.gd`, `BoardManager.gd`, `CardManager.gd`, `ProfileManager.gd`)**:
        ...
        *   **보상 선택 UI(RewardUI.gd) 프로모션 스타일 개편 및 4장 확장**:
            ...
            *   **최종 3스테이지 클리어 시 특수 선택지 구현 (`_on_restart_with_current_deck_pressed`)**: 강화된 덱을 그대로 소유한 채 1스테이지부터 다시 도전하는 **`[ 덱 유지하고 1스테이지 재도전 ]`** 버튼 기능 추가.
            *   **UI 텍스트 정돈**: 카드 삭제 버튼 및 삭제/획득 결과 안내 메시지에서 불필요한 이모지(🗑️, ✨ 등) 제거 완료.
        *   **손패 3D 카드 오른쪽 기준 드로우 채움 및 100% 수평 정렬 (`CardManager.gd`, `CardVisual3D.gd`)**:
            *   **오른쪽 기준 드로우 채움 (`card_x = start_x - i * spacing`)**: 첫 번째 드로우 카드(`i=0`)가 오른쪽 끝(`+X`)에 배치되고 다음 카드가 왼쪽(`-X`)으로 차례대로 확장 배치.
            *   **100% 완전 수평 baseline 정렬 (`local_pos.z = 0.0`)**: 3D 깊이 차이로 인한 화면 투영 내려앉음 경사도를 제거하고, `sorting_offset` 레이어 지정만으로 겹침을 컨트롤하여 손패가 2D pixel 레벨에서 100% 일직선 수평 정렬되도록 보정.
            *   **여유로운 동적 카드 간격 연동 (`base_spacing = 0.88m`, `max_hand_width = 7.0m`)**: 손패 카드 간 간격을 기존 `0.75m`에서 `0.88m`로 시원하게 확대하여 시각적 가독성과 시원한 간격을 확보하고, 카드 다수 보유 시 최대 7.0m 내 동적 축소 연동.
            *   **최상단 호버 레이어 지정 (`sorting_offset = 100.0`)**: 마우스 호버 시 플레이어 눈앞(`+0.25m Z`) 전진 및 `sorting_offset = 100.0`으로 3D 최상단 무조건 렌더링 연동.
            *   **코스트 표시 UI 위치 및 가독성 개선 (`cost_label`)**: 파란색 상단 텍스트를 좌측 중단 아래(카드덱 3D 이미지 바로 위, `Vector2(45, 570)`)로 이동하고, 색상을 선명한 흰색(`Color.WHITE`)과 검은색 아웃라인으로 가독성 보정.
